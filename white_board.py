...
# Pen & eraser sizes
pen_sizes = [2, 4, 6, 8, 10]
eraser_sizes = [10, 20, 30, 40, 50]
pen_index = 0
eraser_index = 0
...
def change_pen_color():
    ...
    if color:
        drawing_color = color
        using_eraser = False
        line_width = pen_sizes[pen_index]
        update_display()
...
def cycle_pen_size():
    global pen_index, line_width
    pen_index = (pen_index + 1) % len(pen_sizes)
    if not using_eraser:
        line_width = pen_sizes[pen_index]
    update_display()

def toggle_eraser():
    global using_eraser, drawing_color, line_width
    using_eraser = not using_eraser
    if using_eraser:
        drawing_color = canvas_bg
        line_width = eraser_sizes[eraser_index]
    else:
        drawing_color = "black"
        line_width = pen_sizes[pen_index]
    update_display()

def cycle_eraser_size():
    global eraser_index, line_width
...
def update_display():
    ...
    eraser_size_button.configure(text=f"Eraser Size: {eraser_sizes[eraser_index]}")
    pen_size_button.configure(text=f"Pen Size: {pen_sizes[pen_index]}")
...
pen_size_button = ctk.CTkButton(controls, text="Pen Size: 2", command=cycle_pen_size, corner_radius=20)
eraser_button = ctk.CTkButton(controls, text="Eraser: OFF", command=toggle_eraser, corner_radius=20)
eraser_size_button = ctk.CTkButton(controls, text="Eraser Size: 10", command=cycle_eraser_size, corner_radius=20)
